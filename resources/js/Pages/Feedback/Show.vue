...
function formatString(inputString) {
  ...
  if (inputString.includes('@')) {
    ...
    if (words.length >= 2) {
      ...
      inputString = `<strong>${words[0]}</strong> <strong>${words[1]}</strong> ${words.slice(2).join(' ')}`;
    }
  }
                ...
                <div class="" >
                    ...
                    <QuillEditor
                       ...
                       v-model:content="form.textarea"
                        :toolbar="['bold', 'italic', 'underline', 'code-block','mention']"
                    />
                </div>
                <button @click="submitComment(feedback.id)" class="text-white my-2  px-6 rounded bg-gray-900 hover:bg-gray-800 hover:text-white transition-all duration-150 ease-in py-2 hover:border-gray-900 hover:border">Submit Comment</button>
                <hr class="mt-2 mb-2" />
                <div class="mt-4">
                    <!-- Comment 1 -->
                    <div v-for="comment in feedback.comments" class="flex gap-y-3 my-3 items-center space-x-2">
                        <div class="border p-2 w-full border-gray-500 rounded" >
                            ...
                            <p class="text-gray-700 text-sm ml-2" v-html="formatString(comment.text)"></p>
                            <p class="text-gray-700 text-sm ml-2" v-html="convertComment(comment.content)"></p>
                        </div>
                    </div>
                </div>
            </div>
